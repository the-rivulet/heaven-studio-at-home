...
async function kittiesCloseUpClap(delay: number, keepKittiesShown = false) {
  hideKitties();
  // Now show them again
  ...
  await sleep(delay * 3).then(() => { tr.style.top = "0%"; tr.style.right = "0%"; })
...
async function kittiesClap(delay: number, keepKittiesShown = false) {
  hideKitties();
  // Now show them again
  ...
  await sleep(delay * 3).then(() => { m.style.bottom = "0%"; })
...
async function kittiesSpin(delay: number, keepKittiesShown = false, extraWait = 0) {
...
async function kittiesFish(delay: number, keepKittiesShown = false) {
