...
function hitNote(key = "", invert = false) {
    ...
    let offset = note.time - Date.now() + (parseFloat(getId("offset").value) || 0);
    if (offset > 200)
...
function cleanUp(songID, maxScore) {
    ...
    setTimeout(() => {
        ...
        if (missed == false)
            rr.innerHTML += " <img src='assets/ranks/nomiss.png'>";
    }, 10000);
...
function playKarateMan() {
    playSong("karateman", [
        d => {
            ...
            setTimeout(() => { getId("arti").src = "assets/karateman/uppercut.png"; }, 1700);
            setTimeout(() => { getId("arti").src = "assets/karateman/idle.png"; }, 2500);
        },
